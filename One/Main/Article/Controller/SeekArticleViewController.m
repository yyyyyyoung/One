...
@interface SeekArticleViewController ()
{
...
@implementation SeekArticleViewController
...
- (void)loadData
{
    ...
    NSString *timeSpNow = [NSString stringWithFormat:@"%ld", (long)[datenow timeIntervalSince1970]];
    
    CGFloat timesp = [timeSpNow floatValue] - [_timeSp floatValue];
    CGFloat dayDif = timesp / 24 / 60 / 60 + 1;
    NSInteger dayyyyyyyyyy = (NSInteger)dayDif;
    NSString *dayDiff = [NSString stringWithFormat:@"%li",dayyyyyyyyyy];

    
    NSString *urlStr = [NSString stringWithFormat:@"http://rest.wufazhuce.com/OneForWeb/one/getC_N?strDate=%@&strRow=%@&strMS=1",date,dayDiff];
//        NSString *urlStr =@"http://rest.wufazhuce.com/OneForWeb/one/getC_N?strDate=2015-10-30&strRow=&strMS=2";
...
{
    _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(5, 64, kScreenWidth - 10, kScreenHeight - 64)];
